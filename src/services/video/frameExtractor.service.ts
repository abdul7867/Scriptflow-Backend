 ...
 * 
 * Performance Optimization PRD Section 3.2.1-3.2.2:
 * - Reduced frame count to 6-8 (from 12-16)
 * - Lowered resolution to 360p (from 480p)
 * - Adjusted quality to 8 (from 5) for smaller file sizes
 * 
...
export async function extractFrames(
  ...
  // Performance Optimization PRD Section 3.2.1-3.2.2:
  // quality 8 (was 5), width 360 (was 480) - reduces file size by ~50%
  const {
    quality = 8,   // Increased from 5 for smaller files (still readable)
    width = 360    // Decreased from 480 for faster processing
  } = options;

  // Maximum frames capped at 8 (was 20) per Performance PRD
  const MAX_FRAMES = 8;
  ...
  try {
    ...
    const expectedFrames = Math.min(Math.ceil(duration * fps), MAX_FRAMES);
    ...
    await new Promise<void>((resolve, reject) => {
      ffmpeg(videoPath)
        .outputOptions([
          ...
          `-frames:v ${MAX_FRAMES}`             // Hard cap at MAX_FRAMES (reduced from 20)
        ])
